...
test('a mocked response flow preserves submitted name and polls without duplicate generation',async({page})=>{
  ...
  await page.route('**/api/name-logo/catalog',route=>route.fulfill({json:{enabled:true,sitekey:'test',styles:[{mode:'logo',id:'hard-angular',name:'Diagonal Weave'}]}}));
  ...
  await page.getByRole('button',{name:/Diagonal Weave/}).click();await page.getByRole('button',{name:'Generate',exact:true}).click();
  await expect(page.getByText('1 of 1 designs finished')).toBeVisible();
...
test('lost admission response can replay same saved request after reload',async({page})=>{
 ...
 await page.goto('/#logo');await page.locator('#first-name').fill('Natia');await page.locator('#last-name').fill('Odisharia');await page.getByRole('button',{name:/Diagonal Weave/}).click();await page.getByRole('button',{name:'Generate',exact:true}).click();await expect(page.getByText(/Submission could not be confirmed/)).toBeVisible();await page.reload();await page.getByRole('button',{name:'Recover saved request'}).click();await expect(page.getByText(/Result uncertain/)).toBeVisible();
});
...
test('catalog connection failure stops the loading message and keeps generation disabled',async({page})=>{
 ...
 await page.goto('/#logo');
 await expect(page.getByText('Could not connect. Refresh to try again.')).toBeVisible();
