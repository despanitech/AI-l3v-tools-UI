...
export default function NameLogoGenerator({first,last,visible,onFirst,onLast,intro,onStepChange,onReachable,onActiveSetCleared,buildStep}){
 ...
 return <>
  <section className="live-logo-generator" aria-label="Create your designs">
   ...
   {(!request||viewingPast)&&<>{step==='name'?<><div className="shared-name"><label>Enter your first name<input id="first-name" value={first} maxLength={80} onChange={e=>changeFirst(e.target.value)}/></label><label>Enter your last name<input id="last-name" value={last} maxLength={80} onChange={e=>changeLast(e.target.value)}/></label></div>{nameContinue}{request&&<button type="button" className="identity-start-again" onClick={startAgain}>Start a new identity with this name</button>}</>:config&&<StyleCatalog first={first} last={last} styles={config.styles||[]} selected={selected} onChange={changeStyles} onEdit={()=>{setStep('name');if(request)setManualStep(1)}} security={generationSecurity} footer={generationFooter}/>}</>}
