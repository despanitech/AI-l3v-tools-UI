...
export default function CapacityStatus(){
 ...
 useEffect(()=>{let live=true,timer;const load=()=>fetch('/api/capacity-status',{headers:{Accept:'application/json'}}).then(r=>{if(!r.ok)throw Error();return r.json()}).then(value=>{if(live)setData(value)}).catch(()=>{if(live)setData(null)}).finally(()=>{if(live)timer=setTimeout(load,15000)});load();return()=>{live=false;clearTimeout(timer)}},[]);
