...
export default function SeedanceVideo({frameId,config,requestAccess}) {
  ...
  const duration=5;
  const [resolution,setResolution]=useState(saved?.settings?.resolution ?? '480p'),[audio,setAudio]=useState(saved?.settings?.audio ?? false);
  ...
  const valid=credits<=Number(config.videoMaxCredits||450);
  ...
  return <section aria-label="Seedance video generation"><h3>Animate with Seedance 2.5</h3>
    <div className="video-fixed-duration"><span>Duration</span><strong>5 seconds</strong></div>
