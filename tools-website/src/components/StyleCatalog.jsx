...
import {orderStyles,onPhone} from '../lib/style-order.mjs';
...
export default function StyleCatalog({styles,selected,onChange,first,last,onEdit,security,footer}){
 ...
 <section className="identity-workspace" aria-label="Choose styles">
  ...
  <div className="identity-preview-stage">
   <div className="preview-mode-heading"><span>Cycle through to visualise</span><div className="preview-mode-tabs">{[['all-horizontal','All 3 horizontal'],['all-vertical','All 3 vertical'],...modes].map(([mode,label])=><button key={mode} type="button" aria-pressed={active===mode} onClick={()=>setActive(mode)}>{label}</button>)}</div></div>
