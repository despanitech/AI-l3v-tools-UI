...
export default function StyleCatalog({styles,selected,onChange,first,last,onEdit,security,footer}){
 ...
 return <section className="identity-workspace" aria-label="Choose styles">
  ...
  <div className="identity-preview-stage">
   <div className="preview-mode-heading"><span>Cycle through to visualise</span><div className="preview-mode-tabs">{[...modes,['all','All 3']].map(([mode,label])=><button key={mode} type="button" aria-pressed={active===mode} onClick={()=>setActive(mode)}>{label}</button>)}</div></div>
   <div className={active==='all'?'all-three-preview':'single-preview'}>{active==='all'?modes.map(([mode])=>carousel(mode,true)):carousel(active)}</div>
  </div>
