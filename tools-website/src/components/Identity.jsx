...
export default function Identity({tool, localPreview}) {
  ...
  const f = example ? 'Clara' : first.trim(), l = example ? 'Evans' : last.trim();
  ...
  return <section id="logo-panel" aria-label="Magic Identity" hidden={!visible}>
    <div className="name-workspace"><p className="eyebrow">YOUR NAME, THREE DIRECTIONS</p><h1>Make it your own.</h1>
      <div className="shared-name"><label>First name<input id="first-name" autoComplete="given-name" placeholder="Clara" maxLength={80} value={first} onChange={e => setFirst(e.target.value)} /></label><label>Last name<input id="last-name" autoComplete="family-name" placeholder="Evans" maxLength={80} value={last} onChange={e => setLast(e.target.value)} /></label></div>
