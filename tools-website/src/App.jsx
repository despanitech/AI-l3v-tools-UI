...
const route = () => ['logo', 'initials', 'signature', 'video', 'demo', 'work'].includes(location.hash.slice(1)) ? location.hash.slice(1) : 'home';
export default function App() {
  ...
  useEffect(() => { document.title = tool === 'home' ? 'l3v AI tools' : `${({logo: 'Magic Identity', initials: 'Magic Initials', signature: 'Magic Signature', video: 'AI Video Suggestion', demo:'Magic Identity Demo', work:'My work'})[tool]} · l3v AI tools`; }, [tool]);
  useEffect(() => {
  ...
  return <InvitationGate><a className="skip" href="#workspace">Skip to tool</a><header className="topbar"><a className="brand" href="#home" aria-label="l3v AI tools home"><img src="/assets/l3v-mark.png" alt="" width="54" height="36" /><span>AI tools</span></a><nav aria-label="Tools"><a href="#home" id="nav-home" aria-current={tool === 'home' ? 'page' : undefined}>All tools</a><a href="#logo" id="nav-logo" aria-current={identity ? 'page' : undefined}>Magic Identity</a><a href="#video" id="nav-video" aria-current={tool === 'video' ? 'page' : undefined}>AI Video Suggestion</a><a href="#work" aria-current={tool === 'work' ? 'page' : undefined}>My work</a></nav><Appearance /></header><MenuFontPicker /><main id="workspace" tabIndex={-1}><Home hidden={tool !== 'home'} /><Video hidden={tool !== 'video'} /><MyWork hidden={tool !== 'work'} /><Identity tool={tool} localPreview={localPreview} /></main><footer><span>l3v AI tools</span><span>A little less friction. A little more creating.</span></footer></InvitationGate>;
}
