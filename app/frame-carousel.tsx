...
type Asset={genre:string;url:string;type:string;source:string;model:string;scene:string;credit?:string;license?:string;caveat?:string;frames?:{url:string;fraction:number}[]};
function Frame({asset,fraction,url}:{asset:Asset;fraction:number;url:string}){
 const [failed,setFailed]=useState(false);
 return <figure className="genre-frame">{!failed?<img src={url} alt={asset.scene} loading="lazy" onError={()=>setFailed(true)}/>:<div className="frame-unavailable">This preview could not load. <a href={asset.source} target="_blank" rel="noreferrer">View it at the source ↗</a></div>}<figcaption><strong>{asset.scene}</strong><span>{asset.model} · {asset.type==='video'?'Frame at '+Math.round(fraction*100)+'% of clip':'Product illustration'}</span><a href={asset.source} target="_blank" rel="noreferrer">Original example ↗</a>{asset.credit&&<span>{asset.credit} · <a href={asset.license} target="_blank" rel="noreferrer">License ↗</a></span>}{asset.caveat&&<span>{asset.caveat}</span>}</figcaption></figure>
}
export default function FrameCarousel({genre,name,standalone=false,compact=false}:{genre:string;name:string;standalone?:boolean;compact?:boolean}){
 ...
 const slides=assets.flatMap(asset=>(asset.frames||[]).map(frame=>({asset,...frame}))).slice(0,4);
