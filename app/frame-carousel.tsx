...
export default function FrameCarousel({genre,name,standalone=false}:{genre:string;name:string;standalone?:boolean}){
 ...
 const heading=<div className="frame-heading"><span>FRAME STUDY / {name}</span><span>{standalone?slides.length+' frames':(active+1)+' / '+slides.length}</span></div>;
 return <div className="frame-study">{standalone?<div data-native-frames>{heading}<div className="native-frame-track">{slides.map((s,i)=><div className="native-frame-slide" key={i}><Frame {...s}/></div>)}</div><div className="frame-controls"><button type="button" data-frame-direction="-1" aria-label="Previous frame">←</button><span>Swipe or use the arrows</span><button type="button" data-frame-direction="1" aria-label="Next frame">→</button></div></div>:<Carousel opts={{loop:true}} setApi={setApi} aria-label={name+' sample frames'}>{heading}<CarouselContent>{slides.map((s,i)=><CarouselItem key={i}><Frame {...s}/></CarouselItem>)}</CarouselContent><div className="frame-controls"><CarouselPrevious/><span>Swipe, use arrows, or press ← →</span><CarouselNext/></div></Carousel>}<p className="frame-note">Illustrative source example; may use a different model or version from the recommendation. Filmed references and presenter product illustrations are explicitly labeled. Frames show appearance, not a motion-quality benchmark.</p></div>
}
